...
**Slow playback** is fine now too. When Blender can't keep up it skips frames, and the wiggle used to start over every time it skipped more than 4. Now it keeps going through the skipped frames, even when one frame takes several seconds (a bit approximated, so those frames never replace exact ones in the cache, and a render or a normal replay simulates them properly). The guessed frames only hop back onto exact cached frames of the same run, right after the skip, otherwise the bones would jump to a different wiggle and it looks like a reset. Same thing for renders with a Frame Step.
